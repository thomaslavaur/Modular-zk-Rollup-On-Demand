...
use zksync_types::FullChangeGroupOp;
...
pub struct FullChangeGroupData {
    pub token: u32,
...
pub struct FullChangeGroupWitness<E: RescueEngine> {
...
impl Witness for FullChangeGroupWitness<Bn256> {
    type OperationType = (FullChangeGroupOp, bool);
    type CalculateOpsInput = ();

    fn apply_tx(
        tree: &mut CircuitAccountTree,
        (full_change_group, is_success): &(FullChangeGroupOp, bool),
    ) -> Self {
        let full_change_group = FullChangeGroupData {
            token: *full_change_group.priority_op.token as u32,
            account_address: *full_change_group.priority_op.account_id,
            eth_address: eth_address_to_fr(&full_change_group.priority_op.eth_address),
            group1: full_change_group.priority_op.group1 as u16,
            group2: full_change_group.priority_op.group2 as u16,
            full_exit_amount: full_change_group
                .withdraw_amount
                .clone()
                .map(|amount| fr_from(amount.0))
                .unwrap_or_else(Fr::zero),
            creator_account_id: full_change_group.creator_account_id.unwrap_or_default().0,
            creator_account_address: eth_address_to_fr(
                &full_change_group.creator_address.unwrap_or_default(),
            ),
            nft_serial_id: full_change_group.serial_id.unwrap_or_default(),
            content_hash: full_change_group.content_hash.unwrap_or_default(),
        };

        // le_bit_vector_into_field_element()
        Self::apply_data(tree, &full_change_group, *is_success)
    }
    ...
    fn get_pubdata(&self) -> Vec<bool> {
        ...
        resize_grow_only(
            &mut pubdata_bits,
            FullChangeGroupOp::CHUNKS * CHUNK_BIT_WIDTH,
            false,
        );
        pubdata_bits
    }
    ...
    fn get_offset_commitment_data(&self) -> Vec<bool> {
        let mut commitment = vec![false; FullChangeGroupOp::CHUNKS * 8];
        commitment[7] = true;
        commitment
    }
...
impl FullChangeGroupWitness<Bn256> {
    fn apply_data(
        tree: &mut CircuitAccountTree,
        full_change_group: &FullChangeGroupData,
        is_success: bool,
    ) -> Self {
        ...
        let (audit_path_before, audit_balance_path_before) = get_audits(
            tree,
            full_change_group.account_address,
            full_change_group.token,
        );

        let capacity = tree.capacity();
        assert_eq!(capacity, 1 << account_tree_depth());
        let creator_account_id_fe = fr_from(full_change_group.creator_account_id);
        let serial_id_fe = fr_from(full_change_group.nft_serial_id);
        let account_address_fe = fr_from(full_change_group.account_address);
        let token_fe = fr_from(full_change_group.token);
        let group1 = fr_from(full_change_group.group1);
        let group2 = fr_from(full_change_group.group2);
        ...
        let (audit_special_account, audit_balance_special_account) =
            get_audits(tree, NFT_STORAGE_ACCOUNT_ID.0, full_change_group.token);
        let (
        ...
        let (audit_creator_account, audit_balance_creator_account) = get_audits(
            tree,
            full_change_group.creator_account_id,
            full_change_group.token,
        );
        let (
        ...
        let content_hash_as_vec: Vec<Option<Fr>> = full_change_group
            .content_hash
            .as_bytes()
            .iter()
            .map(|input_byte| {
        ...
        FullChangeGroupWitness {
            ...
            args: OperationArguments {
                eth_address: Some(full_change_group.eth_address),
                full_amount: Some(full_change_group.full_exit_amount),
                group: Some(group1),
                group2: Some(group2),
                special_eth_addresses: vec![
                    Some(full_change_group.creator_account_address),
                    Some(Fr::zero()),
                ],
            ...
            tx_type: Some(fr_from(&FullChangeGroupOp::OP_CODE)),
        }
    }
}
