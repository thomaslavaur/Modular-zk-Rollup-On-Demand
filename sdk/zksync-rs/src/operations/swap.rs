    ...
    P: Provider + Clone,
{
    ...
    pub async fn tx(self) -> Result<(ZkSyncTx, Option<PackedEthSignature>), ClientError> {
        ...
        self.wallet
            .signer
            .sign_swap(group, nonce, orders, amounts, fee, fee_token)
            .await
    ...
    ) -> Result<SyncTransactionHandle<P>, ClientError> {
        let provider = self.wallet.provider.clone();

        let (tx, eth_signature_swap) = self.tx().await?;
